...
from data.utils import polygons2masks, polygons2masks_overlap
from utils import LOGGER, IterableSimpleNamespace, colorstr
from utils.checks import check_version
from utils.instance import Instances
from utils.metrics import bbox_ioa
from utils.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from data.augment import Compose, RandomHSV, RandomFlip
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from data.augment import RandomHSV
    ...
    def __call__(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # lut_hue = ((x * (r[0] + 1)) % 180).astype(dtype)   # original hue implementation from upstream YOLO<=8.3.78
    ...
    def __call__(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            labels (Dict[str, Any]): A dictionary containing the following keys:
                ...
                'instances' (utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (Dict[str, Any]): The same dictionary with the flipped image and updated instances:
                'img' (np.ndarray): The flipped image.
                'instances' (utils.instance.Instances): Updated instances matching the flipped image.
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
def classify_transforms(
    ...
    """
    import torchvision.transforms as T  # scope for faster 'import lfan'
