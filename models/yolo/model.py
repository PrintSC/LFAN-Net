...
from data.build import load_inference_source
from engine.model import Model
from models import yolo
from nn.tasks import DetectionModel
from utils import ROOT, YAML
...
class YOLO(Model):
    ...
    def __init__(self, model: Union[str, Path] = "yolo11n.pt", task: Optional[str] = None, verbose: bool = False):
        ...
        if ("-world" in path.stem or "yoloe" in path.stem) and path.suffix in {".pt", ".yaml", ".yml"}:
            raise NotImplementedError("This minimal LFAN-Net release keeps only standard detection training.")
