...
from utils.patches import override_configs
...
class DetectionTrainer(BaseTrainer):
    ...
    def get_model(self, cfg: Optional[str] = None, weights: Optional[str] = None, verbose: bool = True):
        ...
        if weights:
            model.load(weights)
        elif getattr(model, "is_fan", False) and self.args.pretrained is True:
            scale = model.yaml.get("scale") or next(iter(model.yaml.get("scales", {"n": None})))
            base_weights = Path(f"yolo11{scale}.pt")
            try:
                weights, _ = attempt_load_one_weight(base_weights)
                model.load(weights)
                LOGGER.info(f"Loaded FAN backbone initialization from '{base_weights}'.")
            except Exception as e:
                LOGGER.warning(f"Unable to auto-load pretrained weights '{base_weights}' for FAN model: {e}")
        return model
    ...
    def get_validator(self):
        ...
        self.loss_names = (
            ("box_loss", "cls_loss", "dfl_loss", "freq_metric", "task_metric", "sem_metric")
            if getattr(model, "is_fan", False)
