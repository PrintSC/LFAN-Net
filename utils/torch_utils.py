...
def get_cpu_info():
    ...
    from utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    # Auto-select GPUs
    if "-1" in device:
        from utils.autodevice import GPUInfo
...
def model_info_for_loggers(trainer):
    ...
    Args:
        trainer (engine.trainer.BaseTrainer): The trainer object containing model and validation data.
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/lfan/yolov5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Examples:
        >>> from utils.torch_utils import profile_ops
