...
from nn.modules import (
    ...
    YOLOEDetect,
    YOLOESegment,
    v10Detect, Pzconv, FCM_3, FCM_2, FCM_1, FCM, Down, SCAM, FFM_Concat2, FFM_Concat3, C3, SPPF, FEM, CIM,
    LargeKernelBlock
)
from utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, YAML, colorstr, emojis
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        self.is_fan = bool(self.yaml.get("fan", False))
    ...
    def _predict_augment(self, x):
        ...
        for si, fi in zip(s, f):
            ...
            yi = (self._predict_fan(xi)[0] if getattr(self, "is_fan", False) else super().predict(xi)[0])  # forward
            yi = self._descale_pred(yi, fi, si, img_size)
            y.append(yi)
        y = self._clip_augmented(y)  # clip augmented tails
        return torch.cat(y, -1), None  # augmented inference, train

    def predict(self, x, profile=False, visualize=False, augment=False, embed=None, prior=None, return_aux=False, guide_mix=0.0):
        """Run default prediction or the FAN-aware forward path."""
        if not getattr(self, "is_fan", False):
            return super().predict(x, profile=profile, visualize=visualize, augment=augment, embed=embed)
        if augment:
            return self._predict_augment(x)
        return self._predict_fan(
            x, profile=profile, visualize=visualize, embed=embed, prior=prior, return_aux=return_aux, guide_mix=guide_mix
        )

    def _predict_fan(self, x, profile=False, visualize=False, embed=None, prior=None, return_aux=False, guide_mix=0.0):
        """Forward pass that injects box priors into FAN blocks."""
        y, dt, embeddings = [], [], []
        fan_aux = {"blocks": []}
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        for m in self.model:
            ...
            if isinstance(m, FANBlock):
                x = (
                    m(x, prior=prior, guide_mix=guide_mix)
                    if not return_aux
                    else m(x, prior=prior, return_cache=True, guide_mix=guide_mix)
                )
                if return_aux:
                    x, block_aux = x
                    fan_aux["blocks"].append(block_aux)
            else:
                x = m(x)
            ...
            if m.i in embed and isinstance(x, torch.Tensor):
                ...
                if m.i == max_idx:
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        return (x, fan_aux) if return_aux else x

    def loss(self, batch, preds=None):
        """Compute detection loss or the FAN-aware joint loss."""
        if not getattr(self, "is_fan", False):
            return super().loss(batch, preds)
        if getattr(self, "criterion", None) is None:
            self.criterion = self.init_criterion()
        if preds is None:
            from nn.modules import build_box_prior

            if not hasattr(self, "fan_forward_steps"):
                self.fan_forward_steps = 0
            self.fan_forward_steps += 1
            guide_warmup = max(int(getattr(self.args, "fan_guide_warmup", 8000)), 1)
            guide_max_mix = float(getattr(self.args, "fan_pred_mix", 0.08))
            if guide_max_mix > 0:
                warmup_ratio = min(self.fan_forward_steps / guide_warmup, 1.0)
                guide_mix = guide_max_mix * (warmup_ratio * warmup_ratio)
            else:
                guide_mix = 0.0
            ...
            preds = self.forward(batch["img"], prior=prior, return_aux=True, guide_mix=guide_mix)
        return self.criterion(preds, batch)
    ...
    def init_criterion(self):
        ...
        if getattr(self, "is_fan", False):
            return FANDetectionLoss(self)
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from nn.text_model import build_text_model
...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from utils.loss import TVPDetectLoss
...
class YOLOESegModel(YOLOEModel, SegmentationModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from utils.loss import TVPSegmentLoss
...
def torch_safe_load(weight, safe_only=False):
    ...
    try:
        with temporary_modules(
                modules={
                    "lfan.yolo.utils": "utils",
                    "lfan.yolo.v8": "models.yolo",
                    "lfan.yolo.data": "data",
                },
                attributes={
                    "nn.modules.block.Silence": "torch.nn.Identity",  # YOLOv9e
                    "nn.tasks.YOLOv10DetectionModel": "nn.tasks.DetectionModel",  # YOLOv10
                    "utils.loss.v10DetectLoss": "utils.loss.E2EDetectLoss",  # YOLOv10
                },
        ):
            if safe_only:
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"with https://github.com/lfan/yolov5.\nThis model is NOT forwards compatible with "
    ...
    base_modules = frozenset(
        {
            ...
            SCDown,
            C2fCIB,
            A2C2f, Pzconv, FCM_3, FCM_2, FCM_1, FCM, Down, C3, FEM, CIM, LargeKernelBlock, FANBlock
        }
    )
